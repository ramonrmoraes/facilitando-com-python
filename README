# Facilitando com Python

## Demonstração da automação

Abaixo está uma demonstração do robô realizando o cadastro automático de produtos no sistema.

![Demonstração da automação](demo-automacao.gif)

Projeto de automação desenvolvido em Python para cadastrar produtos automaticamente em um sistema web.

## Tecnologias utilizadas

* Python
* PyAutoGUI
* Pandas

## Funcionalidades

O robô realiza automaticamente:

1. Abertura do navegador
2. Acesso ao sistema
3. Login automático
4. Leitura de uma base de dados CSV
5. Cadastro automático de produtos

## Estrutura do projeto

facilitando-automacao-python

automacao_cadastro_produtos.py → código da automação  
produtos.csv → base de dados dos produtos  
README.md → documentação do projeto  

## Objetivo

Projeto desenvolvido para prática de automação de tarefas utilizando Python.

## Observação

Este projeto foi desenvolvido apenas para fins educacionais.  
O site utilizado na demonstração pertence à Hashtag Treinamentos e é utilizado nos cursos da plataforma para prática de automação de processos.
